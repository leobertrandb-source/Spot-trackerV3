...
export default function PlayertekLinkModal({ provider = 'playertek', players = [], onClose, onSaved }) {
  ...
  useEffect(() => {
    ...
    async function loadAthletes() {
      ...
      try {
        ...
        if (active) {
          setAthletes([])
          setErrorMessage("Impossible de charger les athlètes.")
        }
      } finally {
  ...
  const uniquePlayers = useMemo(() => {
    ...
    for (const row of players) {
      ...
      const suggestion =
        normalizedAthletes.find((athlete) => athlete.normalized_name === normalizeName(playerName)) || null
  ...
  async function handleSave() {
    ...
    if (!mappings.length) {
      setErrorMessage("Aucune liaison sélectionnée.")
      return
    }
    ...
    try {
      const { data, error } = await supabase.functions.invoke('link-playertek-athletes', {
        body: {
          coachId,
          provider,
          mappings,
      ...
      if (onSaved) onSaved(data?.stats || null)
    } catch (error) {
      console.error(error)
    ...
    >
        ...
        >
          ...
          <div style={{ display: 'grid' }}>
            {uniquePlayers.map((player, index) => (
              <div
                key={`${player.external_id}-${player.player_name}-${index}`}
                style={{
                  display: 'grid',
                  gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)',
                  gap: 14,
                  padding: 14,
                  borderBottom: index === uniquePlayers.length - 1 ? 'none' : `1px solid ${T.border}33`,
                  alignItems: 'center',
                }}
              >
                <div>
                  ...
                  <div style={{ fontSize: 11, color: T.textDim, marginTop: 4 }}>
                    ID externe : {player.external_id || 'non fourni'}
                  </div>
                </div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setSelectedMap((prev) => ({
                      ...prev,
                      [player.external_id || player.player_name]: e.target.value,
                    }))
                  }
                  style={{
                    width: '100%',
      ...
      >
        ...
        <button
          type="button"
          onClick={handleSave}
          disabled={saving || loadingAthletes}
          style={{
            height: 44,
            borderRadius: 12,
            border: `1px solid ${T.accent + '30'}`,
            ...
            color: T.accentLight,
            padding: '0 16px',
            cursor: saving || loadingAthletes ? 'default' : 'pointer',
            fontWeight: 900,
            fontSize: 13,
            opacity: saving || loadingAthletes ? 0.7 : 1,
          }}
        >
