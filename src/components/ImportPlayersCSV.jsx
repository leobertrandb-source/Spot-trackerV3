...
export default function ImportPlayersCSV({ onClose, onSuccess }) {
  ...
  const validRows = useMemo(
    () => rows.filter((row) => row.email && (row.first_name || row.last_name)),
    [rows]
  )
  ...
  async function handleImport() {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_SUPABASE_URL}/functions/v1/import-players-csv`,
        {
          method: 'POST',
          headers: {
      ...
      setSuccessMessage("Import terminé avec invitations envoyées.")
      if (onSuccess) await onSuccess()
    } catch (error) {
      console.error('Import players CSV error:', error)
      setErrorMessage(error?.message || "Erreur lors de l'import des joueurs.")
    } finally {
      setLoading(false)
    }
  }
    ...
    >
      ...
      >
        <div>
          ...
          >
            Format recommandé : email, first_name, last_name. Chaque joueur recevra une invitation par email.
          </div>
        </div>
      ...
      {successMessage ? (
        ...
        >
          {successMessage}
          {importStats
            ? ` ${importStats.created} créé(s), ${importStats.linked} lié(s), ${importStats.invited} invité(s), ${importStats.skipped} ignoré(s).`
            : ''}
        </div>
      ) : null}
