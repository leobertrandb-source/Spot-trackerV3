...
return value == null ? null : Math.round(value * 1.60934 * 100) / 100
}
...
const response = await fetch(`${supabaseUrl}/functions/v1/import-gps-csv`, {
method: 'POST',
headers: {
...
{validRows.length} prête{validRows.length > 1 ? 's' : ''} à importer
</div>
...
color: T.text,
padding: '0 14px',
cursor: !validRows.length ? 'default' : 'pointer',
fontWeight: 800,
fontSize: 12,
opacity: !validRows.length ? 0.7 : 1,
}}
>
...
setSuccessMessage("Liaisons PlayerTek enregistrées. Tu peux relancer l'import GPS.")
}}
/>
</div>
</div>
