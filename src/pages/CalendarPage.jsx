...
function isSameDay(a, b) {
  return toISO(new Date(a)) === toISO(new Date(b))
}
  ...
  const load = useCallback(async () => {
    ...
    if (isCoach) {
      ...
      for (const m of (matches || [])) {
        allEvents.push({
          id: m.id, type: 'match', date: m.match_date,
      ...
      for (const a of (appts || [])) {
        const apptDate = toISO(new Date(a.date_appointment))
        allEvents.push({
      ...
      if (coachId) {
        ...
        for (const a of (appts || [])) {
          allEvents.push({
            id: a.id, type: 'appointment', date: toISO(new Date(a.date_appointment)),
            title: `RDV ${a.type}`,
            subtitle: a.location || '',
            time: new Date(a.date_appointment).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }),
          })
