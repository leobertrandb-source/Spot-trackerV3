  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        { data: hoopers, error: hoopersError },
        { data: compos, error: composError },
      ...
      if (hoopersError) throw hoopersError
      if (composError) throw composError
      if (chargesError) throw chargesError

      // 3) Dernière entrée HOOPER par athlète
      const hooperByUser = {}
      for (const h of hoopers || []) {
        if (!hooperByUser[h.user_id]) hooperByUser[h.user_id] = h
      }

      // 4) Dernière compo par athlète
      const compoByUser = {}
      for (const c of compos || []) {
        if (!compoByUser[c.user_id]) compoByUser[c.user_id] = c
      }

      // 5) Charge de la dernière semaine
      const chargeByUser = {}
      for (const c of charges || []) {
        if (!chargeByUser[c.user_id]) {
          chargeByUser[c.user_id] = { total: 0, sessions: 0 }
        }
      ...
      console.error('PrepDashboard load error:', error)
      setAthletes([])
    } finally {
      setLoading(false)
    }
        ...
        >
          {[
            ...
            <div
              ...
              style={{
                padding: '14px',
                background: glow ? 'rgba(255,69,102,0.06)' : 'rgba(255,255,255,0.03)',
                borderRadius: 14,
                border: `1px solid ${glow ? 'rgba(255,69,102,0.2)' : T.border}`,
              }}
            >
              <div
                style={{
                ...
                <div
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.background = isAlert
                      ? 'rgba(255,69,102,0.07)'
                      : 'rgba(255,255,255,0.04)'
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.background = isAlert
                      ? 'rgba(255,69,102,0.04)'
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {h?.doms_zones && Object.values(h.doms_zones).some((z) => z.level > 0) && (
                        <div style={{ marginTop: 4, fontSize: 11, color: '#ff4566' }}>
                          🩹 DOMS actifs — {Object.values(h.doms_zones).filter((z) => z.level > 0).length} zone
                          {Object.values(h.doms_zones).filter((z) => z.level > 0).length > 1 ? 's' : ''}
                        </div>
                      )}
                    </div>
