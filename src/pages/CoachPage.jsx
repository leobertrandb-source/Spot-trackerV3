...
// ─── SVG Icons ────────────────────────────────────────────────────────────────
const Icon = ({ d, color = T.textMid, size = 18, strokeWidth = 1.8 }) => (
...
function StatBlock({ label, value, sub, color, iconPath, delay = 0 }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 30, fontWeight: 900, color: T.text, lineHeight: 1, letterSpacing: '-1px' }}>{value}</div>
      <div style={{ fontSize: 12, color: T.textMid, marginTop: 6, fontFamily: "'DM Sans',sans-serif", fontWeight: 500 }}>{label}</div>
  ...
  return (
    <Link to={`/coach/client/${client.id}`}
      ...
      style={{
        ...
        background: hov ? 'rgba(167,139,250,0.07)' : 'rgba(255,255,255,0.025)',
        border: `1px solid ${hov ? 'rgba(167,139,250,0.25)' : T.border}`,
        transition: 'all 0.16s ease',
        animation: 'fadeUp 0.4s ease both',
        animationDelay: `${index * 45 + 200}ms`,
      }}>
      <div style={{ width: 38, height: 38, borderRadius: 11, flexShrink: 0, background: 'rgba(167,139,250,0.15)', border: '1px solid rgba(167,139,250,0.25)', display: 'grid', placeItems: 'center', fontSize: 13, fontWeight: 800, color: T.purple, fontFamily: "'Syne',sans-serif" }}>
        {initials(client.full_name)}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: T.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: "'DM Sans',sans-serif" }}>{client.full_name || 'Client'}</div>
        <div style={{ fontSize: 11, color: T.textDim, marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{client.email}</div>
      </div>
      <div style={{ fontSize: 10, fontWeight: 700, color: T.accent, padding: '3px 9px', background: 'rgba(62,207,142,0.1)', border: '1px solid rgba(62,207,142,0.2)', borderRadius: 20, flexShrink: 0 }}>Actif</div>
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={hov ? T.purple : T.textDim} strokeWidth="2.5" strokeLinecap="round" style={{ flexShrink: 0, transition: 'stroke 0.15s' }}>
...
function InviteCard({ userId }) {
  ...
  return (
    <div style={{ ...GLASS_CARD, padding: '22px', display: 'grid', gap: 16 }}>
      <div>
        <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 16, color: T.text }}>Inviter un client</div>
        <div style={{ fontSize: 12, color: T.textDim, marginTop: 3 }}>Génère un lien d'accès sécurisé</div>
      </div>

      <div style={{ display: 'flex', gap: 8 }}>
        <input value={email} onChange={e => setEmail(e.target.value)} placeholder="client@email.com"
          onKeyDown={e => e.key === 'Enter' && generate()}
          style={{ flex: 1, height: 42, borderRadius: 11, background: 'rgba(255,255,255,0.04)', border: `1px solid ${T.border}`, color: T.text, fontSize: 13, padding: '0 14px', outline: 'none', fontFamily: "'DM Sans',sans-serif" }}
        />
        <button type="button" onClick={generate} disabled={loading || !email.trim()} style={{
          ...
          background: email.trim() ? T.accent : 'rgba(62,207,142,0.15)',
      ...
      {link && (
        <div style={{ animation: 'fadeUp 0.3s ease' }}>
          <div style={{ padding: '10px 14px', borderRadius: 10, background: 'rgba(62,207,142,0.05)', border: '1px solid rgba(62,207,142,0.15)', fontSize: 11, color: T.textMid, wordBreak: 'break-all', lineHeight: 1.6, marginBottom: 8 }}>
            {link}
          </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'relative', padding: '32px 30px' }}>
        <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 2.5, textTransform: 'uppercase', color: T.blue, marginBottom: 12, fontFamily: "'DM Sans',sans-serif" }}>
          {greet()}
        </div>
    ...
    <PageWrap>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '8px 0 48px', display: 'grid', gap: 16 }}>
        ...
        <div className="cg-stats">
          <StatBlock label="Clients actifs" value={clients.length} color={T.blue} iconPath={ICONS.users} delay={80} sub="En suivi" />
          <StatBlock label="Séances cette semaine" value={weekSessions} color={T.accent} iconPath={ICONS.flash} delay={130} sub="Total clients" />
          <StatBlock label="Programmes" value="—" color={T.purple} iconPath={ICONS.grid} delay={180} sub="Créés" />
          <StatBlock label="Invitations" value="—" color={T.orange} iconPath={ICONS.mail} delay={230} sub="En attente" />
        </div>
        ...
        <div className="cg-main">
          ...
          <div style={{ display: 'grid', gap: 12, alignContent: 'start' }}>
            ...
            <div className="cg-quick">
              <QuickAction to="/programmes"    label="Programmes"   sub="Créer une séance"   color={T.accent}  iconPath={ICONS.grid}     delay={350} />
              <QuickAction to="/exercices"     label="Exercices"    sub="Bibliothèque"        color={T.orange}  iconPath={ICONS.dumbbell} delay={400} />
              <QuickAction to="/coach/clients" label="Clients"      sub="Voir les fiches"     color={T.purple}  iconPath={ICONS.users}    delay={450} />
              <QuickAction to="/progression"   label="Progression"  sub="Stats globales"      color={T.blue}    iconPath={ICONS.chart}    delay={500} />
            </div>
          </div>
          ...
          <div style={{ ...GLASS_CARD, padding: '22px', display: 'grid', gap: 14, alignContent: 'start' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 16, color: T.text }}>Mes clients</div>
                <div style={{ fontSize: 11, color: T.textDim, marginTop: 2 }}>{clients.length} en suivi actif</div>
              </div>
              <Link to="/coach/clients" style={{ fontSize: 11, fontWeight: 700, color: T.blue, textDecoration: 'none', padding: '5px 12px', borderRadius: 8, border: '1px solid rgba(96,165,250,0.2)', background: 'rgba(96,165,250,0.06)', fontFamily: "'DM Sans',sans-serif" }}>
                Voir tout →
              </Link>
            </div>

            {loading ? (
              <div style={{ padding: 24, textAlign: 'center', color: T.textDim, fontSize: 13 }}>Chargement...</div>
            ) : clients.length === 0 ? (
              <div style={{ padding: '32px 20px', textAlign: 'center', border: `1px dashed ${T.border}`, borderRadius: 14 }}>
                <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke={T.textDim} strokeWidth="1.2" style={{ display: 'block', margin: '0 auto 10px' }}>
                  <circle cx="9" cy="7" r="4"/><path d="M3 21v-2a4 4 0 014-4h4a4 4 0 014 4v2M16 11h6m-3-3v6"/>
                </svg>
                <div style={{ fontSize: 13, color: T.textDim }}>Aucun client encore.</div>
              </div>
            ) : (
              <div style={{ display: 'grid', gap: 6 }}>
                {clients.slice(0, 7).map((c, i) => <ClientRow key={c.id} client={c} index={i} />)}
                {clients.length > 7 && (
                  <Link to="/coach/clients" style={{ textAlign: 'center', fontSize: 12, color: T.blue, textDecoration: 'none', padding: 10, fontWeight: 700 }}>
