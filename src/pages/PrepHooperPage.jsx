  ...
  return (
    <Card>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        ...
        <div style={{ fontSize: 32, fontWeight: 900, color, fontFamily: T.fontDisplay, minWidth: 36, textAlign: 'right' }}>
          {value}
        </div>
      </div>
  ...
  return (
    <div
      style={{
        padding: '12px 14px',
        borderRadius: 14,
        border: `1px solid ${active ? '#ff456640' : T.border}`,
        background: active ? 'rgba(255,69,102,0.04)' : 'rgba(255,255,255,0.02)',
        transition: 'all 0.2s',
      }}
    >
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: active ? 12 : 0 }}>
        ...
        <button
          onClick={() => onChange(key, 'level', active ? 0 : 5)}
          style={{
            padding: '4px 12px',
            borderRadius: 20,
            border: `1px solid ${active ? '#ff456640' : T.border}`,
            background: active ? 'rgba(255,69,102,0.1)' : 'transparent',
            color: active ? '#ff4566' : T.textDim,
            fontSize: 11,
            fontWeight: 700,
            cursor: 'pointer',
          }}
        ...
        <div style={{ display: 'grid', gap: 10 }}>
          <div>
            <div style={{ fontSize: 11, color: T.textDim, marginBottom: 6 }}>
              Intensité de la douleur : <span style={{ color: '#ff4566', fontWeight: 700 }}>{zoneData.level}/10</span>
            </div>
            <input
              type="range"
              min={1}
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {[1, 2, 3].map((v) => (
                <button
                  key={v}
                  onClick={() => onChange(key, 'inquietude', v)}
                  style={{
                    flex: 1,
                    padding: '7px 6px',
                    borderRadius: 10,
                    border: `1px solid ${zoneData.inquietude === v ? `${INQUIETUDE_COLORS[v]}50` : T.border}`,
                    background: zoneData.inquietude === v ? `${INQUIETUDE_COLORS[v]}15` : 'transparent',
                    color: zoneData.inquietude === v ? INQUIETUDE_COLORS[v] : T.textDim,
                    fontSize: 11,
                    fontWeight: 700,
                    cursor: 'pointer',
                  }}
...
export default function PrepHooperPage() {
  ...
  function updateDoms(zoneKey, field, val) {
    setDomsZones((prev) => ({
      ...prev,
      [zoneKey]: { ...(prev[zoneKey] || {}), [field]: val },
    }))
  }
  ...
  async function handleSave() {
    ...
    const { error } = await supabase.from('hooper_logs').upsert(payload, {
  ...
  const zTotal = useMemo(
    () =>
      calcZScore(
        total,
        history.map((h) => ({
          total: h.fatigue + h.sommeil + h.stress + h.courbatures,
        })),
        'total'
      ),
    [total, history]
  ...
  const trendData = useMemo(
    () => history.slice(0, 14).reverse().map((h) => h.fatigue + h.sommeil + h.stress + h.courbatures),
    [history]
  )
        ...
        <Card glow>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
            <div>
              ...
              <div style={{ fontSize: 48, fontWeight: 900, color: scoreInfo.color, fontFamily: T.fontDisplay, lineHeight: 1 }}>
                {total}
              </div>
            ...
            {isCoach && zTotal !== null && (
              <div style={{ textAlign: 'right' }}>
                ...
                <div style={{ fontSize: 28, fontWeight: 900, color: zColor(zTotal), fontFamily: T.fontDisplay }}>
                  {zTotal > 0 ? '+' : ''}
                  {zTotal}
                </div>
            ...
            {isCoach && !sampleSize && history.length > 0 && (
              <div style={{ fontSize: 11, color: T.textDim, textAlign: 'right' }}>
                ...
                <br />
                dès 10 réponses
                <br />({Math.max(0, 10 - history.length)} restantes)
              </div>
            )}
          </div>
          ...
          {TABS.map((t) => (
            <button
              ...
              style={{
                ...
                padding: '9px 4px',
                borderRadius: 10,
                border: `1px solid ${tab === t.key ? `${T.accent}40` : T.border}`,
                background: tab === t.key ? `${T.accent}12` : 'transparent',
                color: tab === t.key ? T.accentLight : T.textDim,
                fontSize: 12,
                fontWeight: 700,
                cursor: 'pointer',
              }}
                    ...
                    >
                      <div>
                        <div style={{ fontSize: 13, color: T.textMid }}>
                          {new Date(`${log.date}T00:00:00`).toLocaleDateString('fr-FR', {
                            weekday: 'short',
                            day: 'numeric',
                            month: 'short',
                          })}
                        ...
                        <div style={{ fontSize: 11, color: T.textDim, marginTop: 2 }}>
                          ...
                          {domsCount > 0 && (
                            <span style={{ marginLeft: 8, color: '#ff4566' }}>
                              {' '}
                              · {domsCount} zone{domsCount > 1 ? 's' : ''} DOMS
                            </span>
                          )}
                        </div>
                      </div>
                          ...
                          (() => {
                            const z = calcZScore(
                              ...
                              history
                                ...
                                .slice(history.indexOf(log) + 1),
                              'total'
                            )

                            return z !== null ? (
                              <div style={{ fontSize: 10, color: zColor(z) }}>
                                z={z > 0 ? '+' : ''}
                                {z}
                              </div>
                            ) : null
