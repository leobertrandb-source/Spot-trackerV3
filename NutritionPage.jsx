...
export default function NutritionPage() {
  ...
  async function addMeal() {
    const hasValue = meal.calories || meal.proteins || meal.carbs || meal.fats || meal.water
    if (!hasValue) {
    ...
    setSaving(true)
    setError('')
    const { data, error: dbErr } = await supabase.from('nutrition_logs').insert({
  ...
  return (
    <PageWrap>
      ...
      <Card>
        ...
        {error && (
          <div style={{ marginBottom: 10, padding: '9px 14px', background: '#1a0808', border: '1px solid #ff3b5c44', borderRadius: 8, fontSize: 13, color: '#ff3b5c' }}>
            {error}
          </div>
        )}
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <Btn onClick={addMeal} disabled={saving}>{saving ? 'Ajout...' : 'Ajouter'}</Btn>
        </div>
      </Card>
